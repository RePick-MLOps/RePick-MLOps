...
# 리포트 데이터에 대한 이진 인코딩
report_industry_encoded = mlb_industry.transform(report_data[['industry']])
report_stock_encoded = mlb_stock.transform(report_data[['stock']])
...
# 사용자와 리포트 간의 유사도 계산
user_report_similarity = cosine_similarity(user_features.drop(['user_id', 'gender', 'age'], axis=1), 
                                           report_features.drop('report_id', axis=1))
